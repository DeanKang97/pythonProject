...
driver = webdriver.Chrome("C:/Users/Myeongha/Documents/chromedriver_win32/chromedriver.exe", options=options)

url = 'https://search.shopping.naver.com/catalog/21595682800?NaPm=ct%3Dkomxzqw8%7Cci%3D31229bd07a77278e69dfa4e60992d482644214e0%7Ctr%3Dslsl%7Csn%3D95694%7Chk%3D44e274a818b99890c39c3a0a555de968ee518922'
...
productname = driver.find_element_by_css_selector('#__next > div > div.style_container__3iYev > div.style_inner__1Eo2z > div.top_summary_title__15yAr > h2');
...
def clickmenu():  # 쇼핑몰 리뷰 버튼 누르기 ,
    ...
    if "쇼핑몰리뷰" in str(driver.find_element_by_css_selector('#snb > ul > li:nth-child(5)').text):
        print(str(driver.find_element_by_css_selector('#snb > ul > li:nth-child(5)').text))
        driver.find_element_by_css_selector('#snb > ul > li:nth-child(5)').click()
    elif "쇼핑몰리뷰" in str(driver.find_element_by_css_selector('#snb > ul > li:nth-child(4)').text):
        print(driver.find_element_by_css_selector('#snb > ul > li:nth-child(4)').text)
...
#    else if(driver.find_element_by_css_selector('#snb > ul > li:nth-child(4)').text=="쇼핑몰리뷰")
 #   driver.find_element_by_css_selector('#snb > ul > li:nth-child(4)').click()
...
def get_rating() : #평점 가져오기
    total_rating = driver.find_element_by_css_selector('#__next > div > div.style_container__3iYev > div.style_inner__1Eo2z > div.top_summary_title__15yAr > div.top_grade__3jjdl')
    #productname = driver.find_element_by_css_selector('#__next > div > div.style_container__3iYev > div.style_inner__1Eo2z > div.top_summary_title__15yAr > h2');

    csv_name = "rating_"+productname+".csv"
    f = open(csv_name, 'w', encoding='utf-8-sig', newline='')
    wr = csv.writer(f)
    total_rating = total_rating.text #평점x.xx -> x.xx로 변환
    total_rating = total_rating[2:]
    wr.writerow([total_rating])
